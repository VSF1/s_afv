...
import exiftool
import exifutil
from devices.sony import sonyDeviceFactory
...
class draw (object) :
    ...
    def start (self,F) :
        ...
        with exiftool.ExifToolHelper() as et:
            exifdata = et.get_metadata(F, "-a")
        #end with
        exif = exifutil.make_exif(exifdata.pop(0))
        if exif.get("EXIF:Make") in ("SONY"):
            camera = sonyDeviceFactory(exif, im)
            camera.render(ax)
            ax.imshow(im)
            plt.draw()
            return
        ...
        if 'MakerNotes:AFType' in exif:
            ...
            if exif.get('MakerNotes:AFType') in ('15-point'):
                for key in sorted(exif.items()):
                    if key[0].startswith('MakerNotes:AFStatus'):
                        ...
                        if key[0] == 'MakerNotes:AFStatusCenterVertical':
                            ...
                            #end if
                            cross = str(int(cross))
                            ax.add_patch(patches.Rectangle((x_center,y_center), r_size, r_size, linewidth = 1,
                            ...
                            txt.set_path_effects([path_effects.Stroke(linewidth=2, foreground='black'), path_effects.Normal()])
                        #end if MakerNotes:AFStatusCenterVertical
                ...
                if 'MakerNotes:AFPointInFocus' in exif:
                    afif = exif.get('MakerNotes:AFPointInFocus')
                    if afif == 'Center (vertical)' :
                        self.afpointinfocus_render(x_c,y_c,rad)
                    if afif == 'Center (horizontal)' :
                        self.afpointinfocus_render(x_c,y_c,rad)
                    if afif == 'Bottom (vertical)' :
                        self.afpointinfocus_render(x_c,y_c+2*spacer,rad)
                    ...
                    if afif == 'Top (horizontal)' :
                        self.afpointinfocus_render(x_c,y_c-2*spacer,rad)
                    if afif == 'Near Left' :
                        self.afpointinfocus_render(x_c-spacer,y_c,rad)
                    if afif == 'Near Right' :
                        self.afpointinfocus_render(x_c+spacer,y_c,rad)
                    if afif == 'Left' :
                        self.afpointinfocus_render(x_c-3.5*spacer,y_c,rad)
                    if afif == 'Right' :
                        self.afpointinfocus_render(x_c+3.5*spacer,y_c,rad)
                    if afif == 'Lower-middle' :
                        self.afpointinfocus_render(x_c,y_c+spacer,rad)
                    if afif == 'Upper-middle' :
                        self.afpointinfocus_render(x_c,y_c-spacer,rad)
                    if afif == 'Lower-left' :
                        self.afpointinfocus_render(x_c-3.5*spacer,y_c+1.6*spacer,rad)
                    if afif == 'Lower-right' :
                        self.afpointinfocus_render(x_c+3.5*spacer,y_c+1.6*spacer,rad)
                    if afif == 'Upper-left' :
                        self.afpointinfocus_render(x_c-3.5*spacer,y_c-1.6*spacer,rad)
                    if afif == 'Upper-right' :
                        self.afpointinfocus_render(x_c+3.5*spacer,y_c-1.6*spacer,rad)
                    if afif == 'Far Left' :
                        self.afpointinfocus_render(x_c-5*spacer,y_c,rad)
                    if afif == 'Far Right' :
                        self.afpointinfocus_render(x_c+5*spacer,y_c,rad)
            ...
            if exif.get('EXIF:Model') in ('ILCA-77M2', 'ILCA-99M2'):
                if 'MakerNotes:AFPointsUsed' in exif:
                    ...
                    if exif.get ('EXIF:Model') == 'ILCA-99M2':
                        ...
                        vspacer = 1.2*r_size
                        hspacer = 2.2*r_size
                    elif exif.get ('EXIF:Model') == 'ILCA-77M2':
            ...
            #RIGHT PART
                    ...
                    if 'G11' in afp_used:
                        ...
                        ax.add_patch(patches.Rectangle((x_c-r_size/2+5.5*hspacer,y_c+2*vspacer-r_size/2),r_size,r_size,linewidth=2,edgecolor = 'w',facecolor='none',alpha =0.3))
            #endif ILCA-99M, A77M2 AF POINTS END
        #endif AFType
        if 'MakerNotes:FacesDetected' in exif and False:
            faces = int(exif.get('MakerNotes:FacesDetected'))
            if faces > 0 :
                self.face_render(exif, '1')
                self.face_render(exif, '2')
                self.face_render(exif, '3')
                self.face_render(exif, '4')
                self.face_render(exif, '5')
                self.face_render(exif, '6')
                self.face_render(exif, '7')
                self.face_render(exif, '8')
            #endif faces > 0
        #endif faces
        if (exif.get ('MakerNotes:FocalPlaneAFPointsUsed')):
            if exif.get ('MakerNotes:FocalPlaneAFPointsUsed') != '(none)':
                ...
                elif exif.get('EXIF:Model') in ('ILCE-6300','ILCE-6500','ILCA-99M2','ILCA-77M2','ILCE-9','DSC-RX10M4', 
                    'DSC-RX100M5','ILCE-7RM3','ILCE-7M3','DSC-RX100M6','ILCE-6400','ILCE-6600','ILCE-6100','DSC-RX0',
                    'DSC-RX0M2', 'MODEL-NAME', 'DSC-RX100M7', 'ILCE-7RM4', 'ILCE-7RM4A', 'ILCE-9M2','ZV-1','ZV-E10',
                    'ILCE-1'):
                    foc = exif.get ('MakerNotes:FocalPlaneAFPointsUsed')
                    if int(foc):
        ...
        #endif
        # Print TEXT
        if exif.get('MakerNotes:AFType') in ('15-point', '19-point', '79-point'):
            txt = ax.text(0.01*xpixels,0.01*ypixels,str(os.path.basename(F))+' ('+str(pos+1)+'/'+str(len(flist))+')\n'
                +exif.get('MakerNotes:AFType')+' focus model detected ('+str(exif.get('EXIF:Model'))+'). Focus Mode: '
                +str(exif.get('MakerNotes:FocusMode'))+'\n'
                +'Note: Number next to AF point represents in-focus estimation.\n'
                +'Less is better (i.e. 0 = in focus; 32768 = out of focus)',
        ...
        if (exif.get ('MakerNotes:FocalPlaneAFPointsUsed')):
            if exif.get('EXIF:Model') in ('ILCE-6000','ILCE-5100','ILCE-7RM2','ILCE-7M2'):
                if exif.get('MakerNotes:AFTracking') == 'Face tracking':
                    txt = ax.text(0.01*xpixels,0.01*ypixels,str(os.path.basename(F))
                        +' ('+str(pos+1)+'/'+str(len(flist))+')\n'
                        +'Model with Focal Plane AF Points detected ('+str(exif.get('EXIF:Model'))+'). Focus Mode: '
                        +str(exif.get('MakerNotes:FocusMode'))+'\n'
                        +'Focal Plane AF points used = '+str(len(foc))+'\n'
                        +'EYE AF or Face Tracking engaged!',
                        color='y', weight='bold', fontsize='small', ha='left', va='top')
                    txt.set_path_effects([path_effects.Stroke(linewidth=2, foreground='black'), path_effects.Normal()])
                else:
                    txt = ax.text(0.01*xpixels,0.01*ypixels,str(os.path.basename(F))
                ...
                'ILCE-7RM2','ILCE-7M2','DSC-RX100M5','ILCE-7RM3','ILCE-7M3','DSC-RX100M6','ILCE-6400', 'ILCE-6100',
                'ILCE-6600', 'DSC-RX0', 'DSC-RX0M2', 'MODEL-NAME', 'DSC-RX100M7', 'ILCE-7RM4', 'ILCE-7RM4A', 'ILCE-9M2',
                'ZV-1','ZV-E10','ILCE-1'):
                if exif.get('MakerNotes:AFAreaMode') == 'Tracking' and exif.get('MakerNotes:AFTracking') == 'Lock On AF': #and exif.get('Camera Model Name') in ('ILCE-6400','ILCE-9','ILCE-7RM4','ILCE-RX100M7'):
                    ...
                    txt.set_path_effects([path_effects.Stroke(linewidth=2, foreground='black'), path_effects.Normal()])
                elif exif.get('MakerNotes:AFAreaMode') == 'Animal Eye Tracking': # and exif.get('AF Tracking') == 'Lock On AF' and exif.get('Camera Model Name') in ('ILCE-6400','ILCE-9','ILCE-7RM4','ILCE-RX100M7'):
                    txt = ax.text(0.01*xpixels,0.01*ypixels,str(os.path.basename(F))+' ('+str(pos+1)+'/'+str(len(flist))+')\n'+'Model with Focal Plane AF Points detected ('+str(exif.get('EXIF:Model'))+'). Focus Mode: '+str(exif.get('MakerNotes:FocusMode'))+'\nFocal Plane AF points used = '+str(foc)+'\n'+'Animal Eye AF used', color='y', weight='bold', fontsize='small', ha='left', va='top')
                    txt.set_path_effects([path_effects.Stroke(linewidth=2, foreground='black'), path_effects.Normal()])
                elif exif.get('MakerNotes:AFTracking') == 'Face tracking' and exif.get('MakerNotes:AFAreaMode') == ('Tracking'):
                    ...
                    txt.set_path_effects([path_effects.Stroke(linewidth=2, foreground='black'), path_effects.Normal()])
                elif exif.get('MakerNotes:AFTracking') == 'Face tracking' and exif.get('MakerNotes:AFAreaMode') == ('Face Tracking'):
